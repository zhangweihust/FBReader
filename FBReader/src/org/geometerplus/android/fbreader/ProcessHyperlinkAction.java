...
public class ProcessHyperlinkAction extends FBAndroidAction {
	public ProcessHyperlinkAction(FBReaderInterface baseActivity, FBReaderApp fbreader) {
		...
		if (soul instanceof ZLTextHyperlinkRegionSoul) {
			...
			if (url != null) {
				try {
					...
					intent.setClass(BaseActivity.getActivity(), ImageViewActivity.class);
					intent.setData(Uri.parse(url));
					intent.putExtra(
						ImageViewActivity.BACKGROUND_COLOR_KEY,
						Reader.ImageOptions.ImageViewBackground.getValue().intValue()
					);
					OrientationUtil.startActivity(BaseActivity.getActivity(), intent);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			...
			DictionaryUtil.openWordInDictionary(
				BaseActivity.getActivity(), ((ZLTextWordRegionSoul)soul).Word, region
			);
		}
	}
	...
	private void openInBrowser(final String url) {
		...
		if (BookDownloader.acceptsUri(Uri.parse(url))) {
			intent.setClass(BaseActivity.getActivity(), BookDownloader.class);
		...
		new Thread(new Runnable() {
			public void run() {
				if (!url.startsWith("fbreader-action:")) {
					try {
						nLibrary.initialize(new ActivityNetworkContext(BaseActivity.getActivity()));
					} catch (ZLNetworkException e) {
						e.printStackTrace();
						UIUtil.showMessageText(BaseActivity.getActivity(), e.getMessage());
						return;
					}
				}
